...
def test_get_recipes_one_saved_recipe(client, one_recipe):
    ...
    new_recipe = Recipe.query.get(1)
    assert new_recipe
    assert new_recipe.hash == "6cddaef7d51e5ee16fd39363d2355286"
...
# @pytest.mark.skip(reason="No way to test this feature yet")
def test_get_recipe_bad_data(client):
    # Act
    response = client.get("/recipes/chicken")
    response_body = response.get_json()

    # Assert
    assert response.status_code == 400
    assert response_body == {
        'error': 'Invalid recipe id: chicken'
    }
...
def test_get_recipes_from_edamam(client):
    ...
    # Assert
    assert response.status_code == 200
    assert "_links" in response_body

@pytest.mark.skip(reason="Skipping due to rate limit")
def test_get_specific_recipe_from_edamam(client):
    # Act
    response = client.get("/search/6cddaef7d51e5ee16fd39363d2355286")
    response_body = response.get_json()

    # Assert
    assert response.status_code == 200
    assert "recipe" in response_body
    assert response_body["recipe"]["label"] == "Slow Cooker Chicken and Dumplings"