...
def test_create_user_recipe(client, one_user, one_recipe):
    ...
    assert user_recipe
    assert user_recipe.uid == "H5F3cCBqWUYfwRSLUMpNcijZmSf2"
...
def test_update_user_recipe_fave(client, one_user, one_recipe, one_user_recipe):
    ...
    assert new_user_recipe.uid == "H5F3cCBqWUYfwRSLUMpNcijZmSf2"
...
def test_get_user_favorite_recipes(client, one_user, one_recipe, one_user_recipe):
    # Act
    response = client.get("/ur/user/H5F3cCBqWUYfwRSLUMpNcijZmSf2/fave")
    response_body = response.get_json()
...
# @pytest.mark.skip(reason="No way to test this feature yet")
def test_get_user_menu_items(client, one_user, one_recipe, one_user_recipe):
    # Act
    response = client.get("/ur/user/H5F3cCBqWUYfwRSLUMpNcijZmSf2/date?start_date=Aug%2001%202022")
    response_body = response.get_json()
